...
# --- Numba Compiled Subroutines (Keep these fast) ---
@njit
...
# --- Worker Target for Multiprocessing Pipeline ---
def run_single_simulation(seeds, mu1, mu2, alpha):
    """Executes a single workflow simulation iteration in memory."""
    r1, r2, r3, r4 = seeds
    ...
    # Handling the non-communicating case cleanly
    if jump_idx < 0:
    ...
    # Handling regular asymmetric communication updates
    if giver == 1:
        recv_min1, recv_min2 = UpdateReceiver(
            res_a2[0].copy(),
            res_a2[1].copy(),
            jump_idx,
        ...
        val1 = 1 if res_a1[2][-1] < res_a1[3][-1] else 0
        val2 = 1 if recv_min1[-1] < recv_min2[-1] else 0
        val3 = 1 if res_a2[2][jump_idx] < res_a2[3][jump_idx] else 0
        record = (
            res_a1[2][-1],
            res_a1[3][-1],
            val1,
        ...
        val2 = 1 if res_a2[2][-1] < res_a2[3][-1] else 0
        val1 = 1 if recv_min1[-1] < recv_min2[-1] else 0
        val3 = 1 if res_a1[2][jump_idx] < res_a1[3][jump_idx] else 0
        record = (
            recv_min1[-1],
            recv_min2[-1],
            val1,
    ...
    return "asymmetric", record


def Communication(mu1: float, mu2: float, alpha: float, outfile: TextIO):
    print(f"Starting Multi-Core Simulation for Alpha: {alpha}...")
    ...
    # Parallelize iterations across every CPU core core via ProcessPoolExecutor
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for idx, future in enumerate(concurrent.futures.as_completed(futures)):
            ...
            if dtype == "asymmetric":
                asymmetric_records.append(record)
            else:
    ...
    # Write the complete arrays out to disk once in a single bulk sweep
    df_asym = pd.DataFrame(asymmetric_records)
    df_neg = pd.DataFrame(neg_jump_records)

    df_asym.to_csv("asymmetric.csv", index=False)
    df_neg.to_csv("asymmetric_negJump.csv", index=False)

    count1 = (
        ((df_asym.iloc[:, 2] == 0) & (df_asym.iloc[:, 5] == 0)).sum()
    ...
    outfile.write(
        fr"{alpha} & {df_asym.shape[0]} & {count1} & {count2 + count1} \\ \hline"
    )
    outfile.write("\n")
    print(f"Finished Alpha {alpha}. Total Rows written: {df_asym.shape[0]}")


# --- Main Runtime Process Entrypoint ---
